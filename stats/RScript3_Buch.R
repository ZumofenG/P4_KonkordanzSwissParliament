...
Sys.setenv(LANG="en")
rm(list=ls())
setwd("/Users/zumofeng/OneDrive/PhD/P5_Parlament-Konkordanz/Data")
...
a_details$a_id_jahr<-substr(a_details$a_id, 3, 4)
...
write.csv(voten, 'voten.csv', row.names=FALSE)
...
library(ngram)
wordcount(voten_fr$v_text, sep = " ", count.function = sum)
wordcount(voten_ge$v_text, sep = " ", count.function = sum)
...
# 2.4.* create a specific dataset for multi-class classification
speech_party_fr <- subset(voten_fr, select=c(party, text))
speech_party_ge <- subset(voten_ge, select=c(party, text))
speech_text_fr <- subset(voten_fr, select=c(text))
speech_text_ge <- subset(voten_ge, select=c(text))
speech_party_fr_2 <- as.character(speech_party_fr)
write.csv(speech_party_fr, file="speech_party_fr.csv", sep=",")
write.csv(speech_party_ge, file="speech_party_ge.csv", sep=",")
write.csv(speech_text_fr, file="speech_text_fr.csv", sep=",")
write.csv(speech_text_ge, file="speech_text_ge.csv", sep=",")

install.packages("xlsx")
write.xlsx(speech_party_fr, file="speech_party_fr.xlsx")
library(xlsx)
